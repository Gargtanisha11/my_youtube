const ExtraButtonSidebar=()=>{
 return (
    <div className="border  border-b-1 border-t-0 border-l-0 border-r-0 border-gray-400 py-5 px-1">
      <div className=" py-3 px-3 flex">
          <img
            className=" w-8"
            src="data:image/png;base64,"
            alt=""
          />
          <button className="pl-5"> Library</button>
        </div>
        <div className="py-2 px-3 flex">
          <img
            className="w-7 "
            src="https://www.seekpng.com/png/detail/301-3019211_history-comments-history-icon-png.png"
            alt=""
          />
          <button className="pl-6">History</button>
        </div>
        <div className="py-3 px-3 flex">
          <img
            className="w-7 h-7 pt-1"
            src="https://static.thenounproject.com/png/106663-200.png"
            alt="subs"
          />
          <button className="pl-5">Watch Later</button>
        </div>
        <div className="py-3 px-3 flex">
          <img
            className="w-8"
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnyB6F5HjIw0mP_NnRteNg-vgqxx5FMuH1hg&usqp=CAU"
            alt="subs"
          />
          <button className="pl-5">Liked Videos</button>
        </div>
      </div>
 );
}

export default ExtraButtonSidebar;